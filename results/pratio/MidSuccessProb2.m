function [average_success_prob_rd, average_success_prob_pg, total_success] = MidSuccessProb2(Npg, Nrd, mp, p, APaging_ratio)
    ...
    for i = 1:test_round
        ...
        Paging_ratio =  vpa((  mp - (    1 / ( 1-  ( 1 - (1/mp))^( 1+  Npg/(Nrd-1) ) ) )     )   /  Npg, 7);
        
        %Paging_ratio =  (  mp - (    1 / ( 1-  ( 1 - (1/mp))^( 1+  Npg/(Nrd-1) ) ) )     )   /  Npg;
        if Paging_ratio > 1
            Paging_ratio = 1;
        ...
        for i = Npg + 1:N,
            if (UE_state(i) == 0)
                ...
                if mp - round(double(Npg*Paging_ratio)) == 0
                     break;
                end
                UE_preamble(i) = unidrnd(mp - round(double(Npg*Paging_ratio))) + round(double(Npg*Paging_ratio));

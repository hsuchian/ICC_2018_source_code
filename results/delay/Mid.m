function [Average_delay, Number_per_round] = Mid(Npg, Nrd, mp ,p)
    N = Npg + Nrd;
    ...
    while (UE_left(end) > 0)
        ...
        Paging_ratio =  (  mp - (    1 / ( 1-  ( 1 - (1/mp))^( 1+  Paging_left(end)/(Random_left(end)-1) ) ) )     )   /  Paging_left(end);
        ...
        for i = Npg + 1:N,
            if (UE_state(i) == 0)
                ...
                UE_preamble(i) = unidrnd(mp - round(Paging_left(end)*Paging_ratio))   + round(Paging_left(end)*Paging_ratio);
